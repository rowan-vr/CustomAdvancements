...
public class AvailableAdvancementsGUI extends InventoryGUI{
	...
	@Override public Inventory getInventory(final Player player, final boolean ignoreHistory) {
		...
		val string = "availableadvancements:" + page;
		if (!ignoreHistory)
		...
		try {
			...
			inventory.setItem(4, createGuiItem(Material.BARRIER, Lang.GUI_AVAILABLE_NONE_NAME.getString(), Lang.GUI_AVAILABLE_NONE_LORE.getString()));
		}
		setBack(22);
		return inventory;
	}
	...
	@Override public void onClick(final InventoryClickEvent event) {
		...
		switch (index) {
			case 18:
				if (page != 1)
					player.openInventory(new AvailableAdvancementsGUI(page - 1, player).getInventory(player, true));
				break;
			case 26:
				if (page != maxPage)
					player.openInventory(new AvailableAdvancementsGUI(page + 1, player).getInventory(player, true));
